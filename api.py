...
from db import list_links, list_profiles

import discord
from fastapi import FastAPI, Header, HTTPException
from pydantic import BaseModel
from config import GUILD_ID, LINKED_ROLE_ID
from config import ROBLOX_API_KEY, ADMIN_LOG_CHANNEL_ID, ANNOUNCE_CHANNEL_ID
from db import (
    init_db,
    get_code,
    delete_code,
...
async def link_confirm(body: LinkConfirmBody, x_api_key: str = Header(default="")):
    ...
    # Expiration: on laisse Roblox gérer si tu veux, ici on fait simple
    # Si tu veux une vraie expiration, tu peux comparer (now - created_at) à CODE_TTL_SECONDS.

    # Empêche de re-link si déjà link côté discord
    ...
    # Empêche de link un roblox déjà lié à quelqu'un
    ...
    await delete_code(code)
        # 🔔 Discord log: player linked
    if DISCORD_BOT is not None:
        ch = DISCORD_BOT.get_channel(int(ANNOUNCE_CHANNEL_ID))
        if ch is not None:
            embed = discord.Embed(
                title="🔗 Account Linked",
                color=0x1ABC9C  # turquoise/vert clair
            )
            embed.add_field(
                name="Discord",
        ...
        # ✅ Give role on link (if member is in the server)
    if DISCORD_BOT is not None:
...
async def admin_pull(limit: int = 50, x_api_key: str = Header(default="")):
    ...
    # rows = SELECT * FROM admin_actions ...
    # table: id, roblox_user_id, action, amount, queued_at, done, done_at, success, result_text
...
async def admin_report(body: AdminActionReportBody, x_api_key: str = Header(default="")):
    ...
    if DISCORD_BOT is not None:
        ...
        if ch is not None:
            color = 0x2ECC71 if body.success else 0xE74C3C  # green / red
...
async def dashboard():
    ...
    # Essaie d’afficher le username Discord via le bot, sinon mention
    async def discord_label(discord_id: int) -> str:
        ...
        if user:
            return f"{html.escape(user.name)}#{html.escape(user.discriminator)} <span class='muted'>({discord_id})</span>"
        return f"<span class='muted'>@unknown</span> <span class='muted'>({discord_id})</span>"
            ...
            <td>
                <div class="grid">
                    ...
                    <div class="card">
                        ...
                        <div class="v">{kills}</div>
                    </div>
                    <div class="card">
                        <div class="k">Tickets</div>
                        <div class="v">{tickets}</div>
                    </div>
                    <div class="card">
                        <div class="k">Robux</div>
                        <div class="v">{robux}</div>
                    </div>
                </div>
